...
class CtrLabjc extends Controleur
{
    ...
    public function __construct(Motif $motif)
    {
        ...
        //$this->routeur->addRoute(new Route($this->nomApplication, "Windows", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}CJSPG_Windows.php"));
        //$this->routeur->addRoute(new Route($this->nomApplication, "Geolocation", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}CJSPG_Geolocation.php"));
        //$this->routeur->addRoute(new Route($this->nomApplication, "History", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}CJSPG_History.php"));
        //$this->routeur->addRoute(new Route($this->nomApplication, "DOM", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}CJSPG_DOM.php"));
        $this->routeur->addRoute(new Route($this->nomApplication, "dropFichier", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}dropFichier.php"));
        //$this->routeur->addRoute(new Route($this->nomApplication, "ProtoAnnuaire", "{p:[0-9]+}", "%([\w]+)\.([\w]+)\%", __DIR__."{$s}vue{$s}ProtoAnnuaire.php"));
    ...
    public function afficherPage($main)
    {
        ...
        //$t['Windows'] =array( 'lien' => $this->routeur->getRoute('Windows')->generateUri() , 'count' => 0 );
        //$t['Geolocation'] =array( 'lien' => $this->routeur->getRoute('Geolocation')->generateUri() , 'count' => 0 );
        //$t['History'] =array( 'lien' => $this->routeur->getRoute('History')->generateUri() , 'count' => 0 );
        //$t['DOM'] =array( 'lien' => $this->routeur->getRoute('DOM')->generateUri() , 'count' => 0 );  
        $t['dropFichier'] =array( 'lien' => $this->routeur->getRoute('dropFichier')->generateUri() , 'count' => 0 );
        //$t['ProtoAnnuaire'] =array( 'lien' => $this->routeur->getRoute('ProtoAnnuaire')->generateUri() , 'count' => 0 );
    ...
    {    
        ...
        $this->afficherPage(compact('body'));
        /**/
    }
    
    public function Windows(array $variables = [])
    {
        $model = null;// Blog::PageResultat(isset($variables['p']) ? $variables['p'] : 1,12);
    ...
    public function DOM(array $variables = [])
    {
        /*
        $model = null;// Blog::PageResultat(isset($variables['p']) ? $variables['p'] : 1,12);
        $main = $this->render("DOM",compact('model'));
        $this->body($main);
        /**/
